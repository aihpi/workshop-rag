...
def _():
    ...
    # This notebook ranks in memory and never opens a collection, so a stopped Qdrant is not a
    # problem here; asking for it would report a failure the participant cannot act on.
    env = setup(require_qdrant=False, strict=False)
    theme.apply_mpl()
